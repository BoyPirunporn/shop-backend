...
public interface CategoryJpaRepository extends DataTablesRepository<CategoryEntity,Long> {
//    @Query("SELECT c FROM CategoryEntity c WHERE c.name LIKE LOWER(c.name) LIKE LOWER(CONCAT('%', :name, '%'))")
    Optional<CategoryEntity> findByNameContainingIgnoreCase(@Param("name") Long name);
    ...
    Page<CategoryEntity> findAllByParentIsNull(Pageable pageable);
    ...
    List<CategoryEntity> getAllCategoryByParentIsNull();
